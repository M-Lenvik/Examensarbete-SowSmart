...
/**
 * Helper functions for validating recommendations and generating warnings.
 * 
 * Data sources:
 ...
 * 
 * These functions check if dates in recommendations lie outside optimal windows
 * (planting windows, harvest windows, move outdoor windows) and generate warnings
 * to inform users when dates are suboptimal.
 * 
 * Results:
 * - getPlantWarnings: Returns PlantWarning[] (array of warnings for dates outside optimal windows)
 * 
 * Uses:
 * - date/date.ts (parseDateIso, subtractDays, normalizeToStartOfDay, formatDateIso)
 * - date/dateValidation.ts (getHarvestWindowDates, getPlantingWindowDates, getMovePlantOutdoorWindowDates)
 * 
 * Used by:
 * - pages/MyGarden.tsx - for showing warnings in garden overview
 * - components/myGarden/MyGardenTaskList/MyGardenTaskList.tsx - for displaying warnings with tasks
 */
...
/**
 * Plant warning type representing a validation warning for a specific date.
 * 
 * Warnings are generated when dates in recommendations lie outside optimal windows.
 * Each warning includes the plant, date, warning type, and a user-friendly message.
 */
 ...
 * 
 * Compares the date's timestamp to the start and end timestamps.
 ...
 * 
 * Used when generating warning messages to show the optimal start date.
 * 
 * @param plant - The plant to get planting window for
 * @param year - The year to use for calculations
 * @param plantingMethod - The planting method ("indoor" or "outdoor")
 * @returns ISO date string for the first day of the planting window, or null if can't determine
 */
